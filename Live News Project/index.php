...
<input type="submit" class="" value="Search"></div><br>
</div>
</form>
...
<?php if ($rec_pag>1) { echo "<a class='w3-bar-item w3-button w3-border' href='index.php?page=".($rec_pag-1)."'>Previous</a>"; } ?>
<?php for($i=1;$i<=$total_page;$i++){  // For Loop start

 ?>
<a class="w3-bar-item w3-button w3-border" style="background-color:<?php if($i==$rec_pag) { echo "#2A5D78";}?>" href="index.php?page=<?php echo $i; ?>"><?php echo $i; ?></a>

<?php
}
?>
<?php if ($total_page>$rec_pag) { echo "<a class='w3-bar-item w3-button w3-border' href='index.php?page=".($rec_pag+1)."'>Next</a>"; } ?>
</div>
